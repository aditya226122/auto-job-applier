...
with tab1:
    ...
    if all_apps:
        ...
        # Extract unique dates from applied_date
        ...
        st.caption(f"Showing **{len(filtered_df)}** records (out of {len(df)} total recorded).")
        ...
        st.markdown(f"### 📸 Visual Confirmation & Proof Gallery {f'({selected_date})' if selected_date != 'All Dates' else ''}")
        ...
        if proof_records:
            for app in proof_records:
                with st.expander(f"✔ Proof: {app.get('title')} @ {app.get('company')} (Ref: {app.get('reference_id', 'Verified')}) - {app.get('applied_date')}"):
                    ...
                    with col_img:
                        if os.path.exists(app.get("screenshot_path", "")):
                            st.image(app.get("screenshot_path"), caption=f"Verified Confirmation Screenshot - {app.get('company')}")
        else:
            st.info(f"No proof screenshots found for the selected filter ({selected_date} / {selected_status}).")
    else:
        st.info("No job applications logged yet. Click 'Trigger Hourly Run' above to start!")

with tab2:
    st.subheader("Live Matched Fresher Opportunities")
    if st.button("🔄 Search New Jobs Now"):
        with st.spinner("Querying job feeds..."):
            fresh_jobs = job_searcher.search_fresher_jobs(limit=15)
            for j in fresh_jobs:
                score, reason = matcher.calculate_match_score(j)
                with st.expander(f"📌 {j.get('title')} - {j.get('company')} ({score}% Match)"):
                    st.write(f"**Location**: {j.get('location')}")
                    st.write(f"**Portal**: {j.get('portal')}")
                    st.write(f"**Match Analysis**: {reason}")
                    st.write(f"**Description**: {j.get('description')}")
                    st.link_button("View Job Listing", j.get("job_url", "#"))
